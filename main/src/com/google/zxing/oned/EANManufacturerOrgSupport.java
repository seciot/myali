...
final class EANManufacturerOrgSupport
{
  ...
  {
    this.a.addElement(paramArrayOfInt);
    this.b.addElement(paramString);
  }

  final String lookupCountryIdentifier(String paramString)
  {
    a();
    ...
    for (int k = 0; k < j; k++)
    {
      int[] arrayOfInt = (int[])this.a.elementAt(k);
      int m = arrayOfInt[0];
      if (i < m)
        return null;
      if (arrayOfInt.length == 1);
      for (int n = m; i <= n; n = arrayOfInt[1])
        return (String)this.b.elementAt(k);
    }
    return null;
  }
}
